...
function runExample(name: string, fn: () => void) {
    try {
        console.log(`--- ${name} ---`);
        fn();
    } catch (err: any) {
...
const $14 = jq.load(html14);
const mapped = $14('span')
    .map(function (this: JqElement, i: number, el: JqElement): number {
