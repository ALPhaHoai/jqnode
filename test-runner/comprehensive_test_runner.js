...
async function runAllTests(options = {}) {
  const { skipNode = config.get('skipNode'), skipBrowser = config.get('skipBrowser'), skipJQuery = config.get('skipJQuery') } = options;
  ...
  console.log(`   Browser tests: ${skipBrowser ? 'SKIP' : 'RUN'} (${config.get('browserEngine')})`);
  ...
  if (allErrors.length > 0) {
    ...
    const uniqueErrors = allErrors.filter((error, index, self) =>
      index === self.findIndex(e => e.file === error.file && e.details === error.details)
    );
