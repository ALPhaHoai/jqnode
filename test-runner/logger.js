...
class Logger {
  ...
  getLevelColor(level) {
    ...
    switch (level) {
      ...
      case LOG_LEVELS.DEBUG: return colors.dim;
      default: return '';
    }
  }
  ...
  resultsSummary(results) {
    ...
    if (!results.skipNode && results.nodeResults) {
      this.info(`   Node.js: ${results.nodeResults.passed}/${results.nodeResults.passed + results.nodeResults.failed} passed`);
    }

    if (!results.skipBrowser && results.browserResults) {
      this.info(`   Browser: ${results.browserResults.passed}/${results.browserResults.passed + results.browserResults.failed} passed`);
    }

    if (!results.skipJQuery && results.jqueryResults) {
      this.info(`   jQuery: ${results.jqueryResults.passed}/${results.jqueryResults.passed + results.jqueryResults.failed} passed`);
    }
