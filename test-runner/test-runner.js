...
class TestRunner {
    ...
    async runJQueryComparisonTestForFile(testFile) {
        ...
        if (!result.success) {
            ...
            logger.info(
                `jQuery comparison test passed: ${testFile} (${parsed.passed}/${parsed.total})`,
            );
        }
    ...
    async processTestFile(testFile) {
        try {
            ...
            if (config.get('includeBrowserUrls')) {
                logger.info(
                    `Browser URL: http://127.0.0.1:8080/browser-test/all-tests/index.html?library=jqnode&file=${encodeURIComponent(fileName)}`,
                );
            }
            ...
            if (
                !jqResult.skipped &&
                browserResult.nodeQuery &&
                browserResult.jQuery &&
                browserResult.nodeQuery.total !== browserResult.jQuery.total
            ) {
                logger.warn(
                    `Test count mismatch: Node.js (${browserResult.nodeQuery.total}) vs jQuery (${browserResult.jQuery.total})`,
                );
            ...
            if (!config.get('skipBrowser')) {
                ...
                if (browserResult.mismatch) {
                    issues.push({
                        type: 'mismatch',
                        file: fileName,
                        nodeQuery: browserResult.nodeQuery,
                        jQuery: browserResult.jQuery,
                    });
                }
                if (browserResult.consoleErrors && browserResult.consoleErrors.length > 0) {
                    issues.push({
                        type: 'console_error',
                        file: fileName,
                        errors: browserResult.consoleErrors,
                    });
                }
            }
            ...
            const browserSuccess =
                config.get('skipBrowser') ||
                (browserResult.error ? false : !browserResult.mismatch);
        ...
        Object.keys(errorGroups).forEach((type) => {
            ...
            errors.forEach((error) => {
                switch (type) {
                    case 'mismatch':
                        logger.info(
                            `📊 ${error.file}: jqnode ${error.nodeQuery.passed}/${error.nodeQuery.total} vs jQuery ${error.jQuery.passed}/${error.jQuery.total}`,
                        );
                        break;
                    case 'console_error':
    ...
    async runAllTests() {
        ...
        if (this.stats.failedTests.length > 0) {
            finalResults.nodeResults.errors = this.stats.failedTests.map((test) => ({
                test,
                type: 'node',
            }));
            finalResults.browserResults.errors = this.stats.failedTests.map((test) => ({
                test,
                type: 'browser',
            }));
            finalResults.jqueryResults.errors = this.stats.failedTests.map((test) => ({
                test,
                type: 'jquery',
            }));
        }
    ...
    async runSingleFile(fileName) {
        ...
        const result = await this.processTestFile({
            relativePath: fileName,
            windowsPath: fileName,
        });
        return result;
    }
