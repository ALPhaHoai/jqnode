...
describe('jQuery method chaining patterns', () => {
    test('should support find() and attr() chaining', () => {
        const chainHtml = `
      <div class="container">
    ...
    `;
        ...
        const allTitlesUpdated = updatedTitles.nodes.every((node, index) => {
    ...
    `;
        ...
        const allPricesDiscounted = featuredPrices.nodes.every(node => $(node).attr('data-discount') === '10%');
        ...
        const allStatusesHighPriority = featuredStatuses.nodes.every(node => $(node).attr('data-priority') === 'high');
