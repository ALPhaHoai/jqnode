...
describe('prevUntil() method', () => {
    ...
    test('prevUntil() should work with filter parameter', () => {
        ...
        const filteredClassNames = prevUntilWithFilter.nodes.map((node: HtmlNode) => (node.attributes?.class as string).split(' ')[1]);
    ...
    test('prevUntil() should work with selector and filter together', () => {
        ...
        const siblingClassNames = prevUntilFiltered.nodes.map((node: HtmlNode) => (node.attributes?.class as string).split(' ')[1]);
    ...
    test('prevUntil() should work with both selector and filter parameters', () => {
        ...
        const middleValues = prevUntilFiltered.nodes.map((node: HtmlNode) => node.children && node.children[0] ? node.children[0].data : undefined);
