...
describe('find() method - Node-Query vs jQuery Comparison', () => {
    ...
    test('find() should locate elements by tag name - jquery-comparison', () => {
        ...
        expect(nqH1Elements.nodes[0].tagName && nqH1Elements.nodes[0].tagName.toLowerCase()).toBe('h1');
    ...
    test('find() should handle nested structures correctly - jquery-comparison', () => {
        ...
        expect(nqNestedSpans.nodes[0].attributes.class).toBe('highlight');
        expect(nqNestedSpans.nodes[1].attributes.class).toBe('normal');
